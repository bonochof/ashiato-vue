...
```
<script>window.onload = function() { document.getElementsByXPath = function(expression, parentElement) { var r = []; var x = document.evaluate(expression, parentElement || document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null); for (var i = 0, l = x.snapshotLength; i < l; i++) { r.push(x.snapshotItem(i)); } return r; }; if (document.domain == "ashiato.kenkenpa.net") { document.getElementsByXPath("/html/body/div[2]")[0].style.display = "none"; } }</script>
```